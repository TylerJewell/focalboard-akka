...
import {getCurrentDerivedView} from './derived'
...
// The filtering, searching and sorting the original does here is done by the server, and
// what arrives is the list of card ids in the order this view shows them. The cards
// themselves still come from the store, because the server's answer is an ordering rather
// than a copy of the data.
//
// There is no local fallback on purpose. RENDERING.md R4: delete the feed and the interface
// must break, because the alternative -- a client that quietly re-derives when the server has
// not answered -- is a poller wearing a stream's clothes, and it would hide exactly the
// disagreements step e exists to find.
export const getCurrentViewCardsSortedFilteredAndGroupedWithoutLimit = createSelector(
    getCurrentBoardCards,
    getCurrentDerivedView,
    (cards, derived) => {
        if (!derived || !cards) {
            return []
        }
        ...
        return derived.orderedCardIds.map((id) => byId[id]).filter((c) => Boolean(c)) as Card[]
    },
)
