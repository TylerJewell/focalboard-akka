...
import {updateCards, getCurrentBoardCards} from '../../store/cards'
import {refreshDerivedView} from '../../store/derived'
...
const BoardPage = (props: Props): JSX.Element => {
    ...
    // What this view shows is asked of the server, not worked out here. It is asked again
    // whenever the board's cards or the view or the search text change -- which is every
    // input the answer depends on, and is what the stream delivers.
    const searchText = useAppSelector(getSearchText)
    const boardCards = useAppSelector(getCurrentBoardCards)
    useEffect(() => {
        if (activeBoardId && activeViewId) {
            dispatch(refreshDerivedView({boardId: activeBoardId, viewId: activeViewId, searchText}))
        }
    }, [activeBoardId, activeViewId, searchText, boardCards])
