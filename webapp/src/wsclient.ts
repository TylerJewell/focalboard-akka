...
class WSClient {
    ...
    // The server sends the board over an event stream rather than a websocket. Everything
    // above this line -- the subscriptions, the change handlers, the notification queue -- is
    // the original's and is unchanged; what differs is where the frames come from.
    ...
    sendSubscribeToTeamCommand(teamId: string): void {
        if (this.client !== null) {
            this.sendCommand({action: ACTION_SUBSCRIBE_TEAM, teamId})
            return
        }
        if (this.streamTeamId !== teamId) {
            this.openStream(teamId)
        }
    }

    sendUnsubscribeToTeamCommand(teamId: string): void {
        if (this.client !== null) {
            this.sendCommand({action: ACTION_UNSUBSCRIBE_TEAM, teamId})
            return
        }
        if (this.streamTeamId === teamId) {
            this.closeStream()
    ...
    open(): void {
        ...
        // The connection itself is opened per team, by sendSubscribeToTeamCommand below, because
        // the stream's address names the team. Registering the state here rather than waiting
        // for a socket keeps the order the original has: open, then subscribe.
        Utils.log('WSClient open: server-sent events')
        this.state = 'open'
    ...
    private openStream(teamId: string): void {
        this.closeStream()
        const source = new EventSource(`${this.getBaseURL()}/api/v2/teams/${teamId}/stream`)
        this.stream = source
        this.streamTeamId = teamId

        source.onopen = () => {
            Utils.log('WSClient stream opened.')
            const wasClosed = this.state === 'close'
            this.state = 'open'
            for (const handler of this.onStateChange) {
                handler(this, 'open')
            }
            if (wasClosed) {
                // The board is re-read rather than caught up from where it left off. That is
                // the original's own policy across a dropped connection, and it is the only one
                // available to a stream that does not replay: what happened while the
                // connection was gone was never sent.
                for (const handler of this.onReconnect) {
                    handler(this)
                }
            }
        ...
        source.onerror = (e) => {
            Utils.logError('WSClient stream error')
            this.state = 'close'
            for (const handler of this.onStateChange) {
                handler(this, 'close')
            }
        ...
        source.onmessage = (e) => {
            if (source !== this.stream) {
                Utils.log('Ignoring closed stream')
                return
            }
            try {
                const frame = JSON.parse(e.data)
                for (const board of frame.boards || []) {
                    this.updateHandler({action: ACTION_UPDATE_BOARD, board, teamId} as WSMessage)
                }
                for (const block of frame.blocks || []) {
                    this.updateHandler({action: ACTION_UPDATE_BLOCK, block, teamId} as WSMessage)
                }
            } catch (err) {
                Utils.logError(`WSClient stream frame is not an object: ${err}`)
            }
        }
    }
    ...
    hasConn(): boolean {
        return this.stream !== null || this.client !== null || this.state === 'open'
    }
    ...
    close(): void {
        ...
        Utils.log('WSClient close')

        // The handlers go first so nothing fires while the stream is being torn down.
