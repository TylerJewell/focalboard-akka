...
class OctoClient {
    ...
    // What this view shows, worked out by the server rather than in this browser. The
    // original has no equivalent route: it ships the blocks and every client derives its own
    // answer from them.
    async getDerivedView(boardId: string, viewId: string, searchText: string): Promise<any> {
        const query = searchText ? `?search=${encodeURIComponent(searchText)}` : ''
        const path = `${this.getBaseURL()}/api/v2/boards/${encodeURIComponent(boardId)}/views/${encodeURIComponent(viewId)}/derived${query}`
