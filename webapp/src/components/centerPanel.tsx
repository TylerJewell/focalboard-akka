...
import {DerivedGroup, getCurrentDerivedView} from '../store/derived'
...
const CenterPanel = (props: Props) => {
    ...
    const derivedView = useAppSelector(getCurrentDerivedView)
    ...
    // The columns, and which card is in which, come from the server's answer rather than from
    // `getVisibleAndHiddenGroups` running here. The cards are looked up locally because the
    // server sends an arrangement of ids, not a copy of the cards.
    const {visible: visibleGroups, hidden: hiddenGroups} = useMemo(() => {
        const byId: {[key: string]: Card} = {}
        for (const card of cards) {
            byId[card.id] = card
        }
        const toGroups = (groups: DerivedGroup[]): BoardGroup[] => groups.map((group) => ({
            option: {id: group.optionId, value: group.label, color: group.color} as IPropertyOption,
            cards: group.cardIds.map((id) => byId[id]).filter((c) => Boolean(c)) as Card[],
        }))
        if (!derivedView) {
            return {visible: [] as BoardGroup[], hidden: [] as BoardGroup[]}
        }
        const vg = toGroups(derivedView.visible)
        const hg = toGroups(derivedView.hidden)
    ...
    }, [cards, derivedView, groupByProperty, boardUsers])
